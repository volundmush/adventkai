...
from adventkai.exceptions import DGScriptError
...
class DGScriptInstance:
    ...
    def __repr__(self):
        return f"<{self.state.name} {self.__class__.__name__}: {self.proto.name}>"

    def __init__(self, handler, proto):
        ...
        self.curr_line = 0
        self.context = 0
        self.lines: list[str] = self.proto.cmdlist
    ...
    async def decrement_timer(self, interval: float):
    ...
    async def execute_block(self, start: int, end: int) -> int:
        ...
        while self.curr_line < end:
            ...
            # cover if
                ...
                if not await self.process_if(line[3:]):
                    print(f"IF {line} failed, checking for else/end...")
                    self.curr_line = await self.find_else_end()
                ...
                self.curr_line = await self.find_end()
                continue
            ...
            # cover while
                ...
                if not await self.process_if(line[6:]):
                    print(f"WHILE {line} IS TRUE")
                    self.curr_line = await self.find_done()
                    continue

            # cover switch
            elif line.startswith("switch "):
                self.depth.append((Nest.SWITCH, self.curr_line))
                self.curr_line = await self.find_case(line[7:])
                continue
                ...
                sub_cmd = await self.var_subst(line)
                ...
                match cmd.lower():
                    ...
                    case "return":
                        if len(cmd_split) > 1:
                            ...
                            return ret_val
                    case "wait":
                        if (wait_time := await self.cmd_wait(sub_cmd)):
                            self.set_state(DGState.WAITING)
    ...
    async def process_if(self, cond: str) -> bool:
        val = await self.eval_expr(cond)
        result = self.truthy(self.maybe_negate(val.strip()))
        print(f"IF {cond} : {result}")
        return result

    async def eval_expr(self, line: str) -> str:
        print(f"EVAL_EXPR: {line}")
        trimmed = line.strip()
        if trimmed.startswith("("):
            m = matching_paren(trimmed, 0)
            if m != -1:
                return await self.eval_expr(trimmed[1:m])
        elif (result := await self.eval_lhs_op_rhs(trimmed)):
            return result
        else:
            return await self.var_subst(trimmed)
    ...
    async def eval_lhs_op_rhs(self, expr: str) -> typing.Optional[str]:
        ...
        for op in self.ops_map.keys():
            try:
                ...
                lhr = await self.eval_expr(lhs)
                rhr = await self.eval_expr(rhs)
    ...
    async def find_else_end(self, match_elseif: bool = True, match_else: bool = True) -> int:
        if not self.depth or self.depth[-1][0] != Nest.IF:
        ...
        while i < total:
            ...
            elif match_elseif and line.startswith("elseif ") and await self.process_if(line[7:]):
    ...
    async def find_end(self) -> int:
        return await self.find_else_end(match_elseif=False, match_else=False)

    async def find_done(self) -> int:
    ...
    async def find_case(self, cond: str) -> int:
        if not self.depth or self.depth[-1][0] != Nest.SWITCH:
            raise DGScriptError("find_case called outside of if! alert a codewiz!")

        res = await self.eval_expr(cond)
    ...
    async def eval_var(self, data: str) -> str:
        ...
        while callable(last_mem) or hasattr(last_mem, "execute"):
            if hasattr(last_mem, "execute"):
                last_mem = await last_mem(self).execute()
            else:
                last_mem = await last_mem(self)
            last_mem = _db_check(last_mem)
    ...
    async def var_subst(self, line: str) -> str:
                ...
                match line[i]:
                    ...
                    case "%":
                        ...
                        if m != -1:
                            ...
                            # so, we recurse...
                            recursed = await self.var_subst(line[i+1:m])
                            # now confident that all nested variables are evaluated...
                            out += await self.eval_var(recursed)
                            i = m + 1
                            continue
                    case _:
                        out += line[i]
    ...
    async def cmd_global(self, cmd: str):
        pass

    async def cmd_context(self, cmd: str):
        pass

    async def cmd_rdelete(self, cmd: str):
        args = cmd.split()
        if len(args) < 2:
    ...
    async def cmd_remote(self, cmd: str):
        args = cmd.split()
        if len(args) < 2:
    ...
    async def cmd_set(self, cmd: str):
        args = cmd.split()
        if len(args) < 2:
    ...
    async def cmd_unset(self, cmd: str):
        args = cmd.split()
        if len(args) != 2:
    ...
    async def cmd_wait(self, cmd: str):
        args = cmd.split()
        if len(args) < 2:
            args.append("1")
    ...
    async def cmd_detach(self, cmd: str):
        pass

    async def cmd_attach(self, cmd: str):
        pass
...
class DGHandler:
    ...
    def attach(self, script_id):
        if (dg := adventkai.LEGACY_TRIGGERS.get(script_id, None)) is not None:
